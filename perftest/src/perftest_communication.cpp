...
static int ethernet_client_connect(struct perftest_comm *comm)
{
	...
	for (t = res; t; t = t->ai_next)
	{
		...
		{
			int count = 100 * 60 * 2;
			while (connect(sockfd, t->ai_addr, t->ai_addrlen) != 0)
			{
				usleep(10000);
				count--;
				if (count <= 0)
...
int establish_connection(struct perftest_comm *comm)
{
	...
	if (comm->rdma_params->use_rdma_cm) //use RDMA_CM to connect each others
	{
	...
	else //default for TCP connector
	{
...
{
	if (write(comm->rdma_params->sockfd, msg, size) != (ssize_t)size)
	{
		perror("client write");
...
{
	if (read(comm->rdma_params->sockfd, recv_msg, size) != (ssize_t)size)
	{
											 ...
											 struct rdma_conn_param *conn_param)
{
	...
	needed = ((user_param->connection_type == UD) && ((user_param->tst == BW && user_param->duplex) || (user_param->tst == LAT)));
												...
												struct perftest_parameters *user_param)
{
	...
	ctx->ah[connection_index] = ibv_create_ah_from_wc(ctx->pd, &wc,
													  (ibv_grh *)(ctx->buf[connection_index]), cm_node->cma_id->port_num);
									...
									struct perftest_parameters *user_param, struct rdma_cm_event *event)
{
	...
	{
		error_message = (char *)"Failed to create AH for RDMA CM connection.";
		goto error;
	}

	if ((user_param->tst == BW && user_param->duplex) || (user_param->tst == LAT))
	{
		rc = rdma_cm_send_ud_connection_parameters(ctx, user_param,
												   connection_index);
		if (rc)
		{
			error_message = (char *)"Failed to send UD connection parameters "
									"to the remote node.";
			goto error;
		}
	}
							...
							struct perftest_parameters *user_param, struct rdma_cm_id *cma_id)
{
	...
	if (user_param->tos != DEF_TOS)
	{
		...
		if (rc)
		{
			error_message = (char *)"Failed to set ToS(Type of Service) option for RDMA "
									"CM connection.";
			goto error;
		}
	}
	...
	if (rc)
	{
		error_message = (char *)"Failed to resolve RDMA CM route.";
						  ...
						  struct perftest_parameters *user_param, struct rdma_cm_id *cma_id)
{
	...
	if (connection_index == 0)
	{
		...
		if (rc)
		{
			error_message = (char *)"Failed to initialize RDMA contexts.";
			goto error;
		}
	}
	...
	if (rc)
	{
		error_message = (char *)"Failed to create QP.";
		goto error;
	}
	...
	if (rc)
	{
		error_message = (char *)"Failed to connect through RDMA CM.";
		goto error;
	}
									   ...
									   struct perftest_parameters *user_param, struct rdma_cm_id *cma_id)
{
	...
	if (rc)
	{
		error_message = (char *)"Failed request UD connection parameters for RDMA CM.";
		goto error_2;
	}
	...
	if (rc)
	{
		error_message = (char *)"Failed to accept RDMA CM connection.";
		goto error_2;
	}
	...
	if (rc)
	{
		error_message = (char *)"Failed to initialize UD connection parameters for RDMA CM.";
		goto error_2;
	}
										   ...
										   struct perftest_parameters *user_param, struct rdma_cm_event *event)
{
	...
	if (rc)
	{
		error_message = (char *)"Failed to establish UD connection for RDMA CM.";
		goto error;
	}
						   ...
						   struct perftest_parameters *user_param)
{
	...
	while (ctx->cma_master.connects_left)
	{
		...
		if (rc)
		{
			error_message = (char *)"Failed to get RDMA CM event.";
			goto error;
		}
		...
		if (rc)
		{
			error_message = (char *)"Failed to handle RDMA CM event.";
			goto ack;
		}
		...
		if (rc)
		{
			error_message = (char *)"Failed to ACK RDMA CM event after handling.";
			goto error;
		}
	}
							 ...
							 struct perftest_parameters *user_param)
{
	...
	if (!ctx->cma_master.disconnects_left || user_param->connection_type == UD)
		return rc;
	...
	for (i = 0; i < user_param->num_of_qps; i++)
	{
		...
		if (rc)
		{
			error_message = (char *)"Failed to disconnect RDMA CM connection.";
			goto error;
		}
	}
							  ...
							  struct perftest_parameters *user_param, struct rdma_addrinfo *hints)
{
	...
	rc = rdma_create_id(ctx->cma_master.channel, &listen_id, &ctx->cma_master,
						(enum rdma_port_space)(hints->ai_port_space));
	if (rc)
	{
							  ...
							  struct pingpong_dest *my_dest, struct pingpong_dest *rem_dest)
{
	...
	{
		error_message = (char *)"Failed to create RDMA CM event channel.";
		goto error;
	}
	...
	if (rc)
	{
		error_message = (char *)"Failed to allocate RDMA CM nodes.";
		goto error;
	}
	...
	if (rc)
	{
		error_message = (char *)"Failed to sync between client and server "
	...
	if (rc)
	{
		error_message = (char *)"Failed to create RDMA CM connection.";
		goto error;
	}
