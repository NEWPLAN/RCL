...
static int ctx_xrcd_create(struct pingpong_context *ctx, struct perftest_parameters *user_param)
{
	...
	tmp_file_name = (user_param->machine == SERVER) ? (char *)SERVER_FD : (char *)CLIENT_FD;
...
{
	ctx->send_cq = ibv_create_cq(ctx->context, tx_buffer_depth * user_param->num_of_qps, NULL, ctx->channel, user_param->eq_num);
	if (!ctx->send_cq)
	{
	...
	if (need_recv_cq)
	{
		ctx->recv_cq = ibv_create_cq(ctx->context, user_param->rx_depth * user_param->num_of_qps, NULL, ctx->channel, user_param->eq_num);
		if (!ctx->recv_cq)
		{
...
int ctx_init(struct pingpong_context *ctx, struct perftest_parameters *user_param)
{
		...
		!user_param->use_xrc && (user_param->tst == LAT || user_param->machine == SERVER || user_param->duplex == ON))
	{
		...
		// 		.max_sge = 1}};
		struct ibv_srq_init_attr attr;
		attr.attr.max_wr = user_param->rx_depth; /* when using sreq, rx_depth sets the max_wr */
		attr.attr.max_sge = 1;
						   ...
						   struct perftest_parameters *user_param, struct rdma_addrinfo *hints)
{
	...
	{
		error_message = (char *)"Failed to allocate memory for RDMA CM nodes.";
		goto error;
	}
	...
	for (i = 0; i < user_param->num_of_qps; i++)
	{
		...
		if (user_param->machine == CLIENT)
		{
			...
			if (rc)
			{
				error_message = (char *)"Failed to create RDMA CM ID.";
				goto error;
			}
		}
	}
...
error:
	while (--i >= 0)
	{
		...
		if (rc)
		{
			error_message = (char *)"Failed to destroy RDMA CM ID.";
			break;
		}
	}
