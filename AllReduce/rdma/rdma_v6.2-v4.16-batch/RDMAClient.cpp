...
void *RDMAClient::poll_cq(void *_id)
{
    ...
    int count_write_finished = 0;
    uint32_t buffer_id = 0;
    uint32_t window_id = ctx->window_id;
...
#define IGNORE_ACK
#ifdef IGNORE_ACK
                        count_write_finished++;
                        //std::cout<<count_write_finished<<std::endl;
                        if (count_write_finished >= MAX_DATA_IN_FLIGHT)
                        {
                            std::cout<<"[post send start] window "<< window_id << ", buffer " << buffer_id <<std::endl;
                            for (int i = 0; i < count_write_finished; i++)
                            {
                                ...
                                buffer_id++;
                                if (buffer_id == MAX_DATA_IN_FLIGHT)
