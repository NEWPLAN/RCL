...
bool client_connect_with_server(struct read_lat_context *ctx,
                                char *server_ip,
                                int server_port);
...
{   
    ...
    // Wait for the send request from the server
    if (!wait_for_wc(&prog_ctx, &wc) || !parse_recv_wc(&wc)) {
        fprintf(stderr, "Fail to get the completed recv request\n");
        goto err;
    }

    // Get the memory information of the server
    struct read_lat_mem *rem_mem = (struct read_lat_mem*)prog_ctx.ctrl_buf;
    print_mem(rem_mem);

    // Generate the first read request
    if (!post_data_read(&prog_ctx, rem_mem)) {
        fprintf(stderr, "Could not post read\n");
        goto err;
    }
    ...
    // Wait for the completions of all the read requests
    int cnt_read_compl = 0;
    while (cnt_read_compl < iters) {
        ...
        while (true) {
            ...
            // Trigger the next read request
            if (++cnt_read_compl < iters && !post_data_read(&prog_ctx, rem_mem)) {
    ...
    printf("%d read requests have completed\n", iters);
    ...
    // Generate a send with immediate request to notify the server of completion of reads
    if (!post_ctrl_send_with_imm(&prog_ctx, TEST_COMPLETION)) {
        fprintf(stderr, "Could not post send with immediate\n");
        goto err;
    }

    // Wait for the completion of the send with immediate request
    if (!wait_for_wc(&prog_ctx, &wc) || !parse_send_wc(&wc)) {
        fprintf(stderr, "Fail to get the completed send with immediate request\n");
        goto err;
    }
