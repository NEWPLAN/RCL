...
bool server_connect_with_client(struct write_lat_context *ctx,
                                int server_port);
...
{   
    ...
    struct write_lat_mem *my_mem = (struct write_lat_mem*)prog_ctx.ctrl_buf;
    ...
    if (!wait_for_wc(&prog_ctx, &wc) || !parse_recv_with_imm_wc(&wc)) {
