...
#define SHOWN_LOG_EVERN_N 100000
...
public:
    ...
    int parse_args(int argc, char **argv)
    {
        const struct option longopts[] =
            {
                ...
                {.name = "master", .has_arg = required_argument, .flag = 0, .val = 'm'},
        ...
        while (iarg != -1)
        {
            ...
            case 's':
                ...
                break;
            case 't':
                traffic_class = strtoul(optarg, NULL, 0);
                break;
            case 'c':
                std::cout << "client: " << optarg << std::endl;
                break;
            case 'm':
                std::cout << "master: " << optarg << std::endl;
                break;
            case 'e':
                use_event = true;
                break;
            case 257:
                DATA_MSG_SIZE = atoi(optarg);
                break;
    ...
    void print_config()
    {
        ...
        if (serve_as_client)
        {
            std::cout << " Serving as client, remoteIP :" << server_name;
        }
        else
        {
        ...
        {
            std::cout << " IP cluster: ";
            for (auto &each_ip : cluster)
                std::cout << each_ip << " ";
            std::cout << std::endl;
        }
        if (num_senders)
            std::cout << " Expected senders: " << num_senders << std::endl;

        std::cout << " Topology: ";
        switch (all_reduce)
        {
        case FULL_MESH_ALLREDUCE:
            std::cout << " Full-Mesh AllReduce" << std::endl;
            break;
        case TREE_ALLREDUCE:
            std::cout << " Tree AllReduce" << std::endl;
            break;
        case DOUBLE_BINARY_TREE_ALLREDUCE:
            std::cout << " Double-Binary-Tree AllReduce" << std::endl;
            break;
        case RING_ALLREDUCE:
        ...
        if (all_reduce == TREE_ALLREDUCE)
            std::cout << " The Tree Allreduce width: " << tree_width << std::endl;
...
public:
    std::string &get_local_ip()
    {
        if (local_ip.length() == 0)
            local_ip = NetTool::get_ip("12.12.12.111", 24);
        return local_ip;
    }
