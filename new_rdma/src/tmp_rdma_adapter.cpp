...
NPRDMAAdapter::NPRDMAAdapter()
{
    ...
    LOG(INFO) << "building NPRDMAAdapter ...";
}

NPRDMAAdapter::~NPRDMAAdapter()
{
    delete ctx;
    LOG(INFO) << "Destroy NPRDMAAdapter...";
}

bool NPRDMAAdapter::init_ctx()
{
    if (!ctx)
    {
...
void NPRDMAAdapter::destroy_ctx()
{
    if (!ctx)
    {
        return;
...
int NPRDMAAdapter::get_rocev2_gid_index()
{
    int gid_index = 2;
...
size_t NPRDMAAdapter::write_exact(int fd, char *buf, size_t count)
{
...
size_t NPRDMAAdapter::read_exact(int fd, char *buf, size_t count)
{
    // current buffer loccation
    char *cur_buf = NULL;
...
bool NPRDMAAdapter::write_dest_info(int fd, struct write_lat_dest *my_dest)
{
    if (!fd || !my_dest)
    {
        return false;
...
bool NPRDMAAdapter::read_dest_info(int fd, struct write_lat_dest *rem_dest)
{
    if (!fd || !rem_dest)
    {
        return false;
...
bool NPRDMAAdapter::init_dest(struct write_lat_dest *dest)
{
...
void NPRDMAAdapter::print_dest(struct write_lat_dest *dest)
{
    if (!dest)
    {
        return;
...
bool NPRDMAAdapter::init_data_mem(struct write_lat_mem *mem)
{
    if (!mem || !ctx)
    {
        return false;
...
void NPRDMAAdapter::print_mem(struct write_lat_mem *mem)
{
    if (!mem)
    {
        return;
...
bool NPRDMAAdapter::post_ctrl_recv()
{
    struct ibv_sge list = {
...
// Return true on success.
bool NPRDMAAdapter::post_ctrl_send()
{
    struct ibv_sge list = {
        .addr = (uintptr_t)(ctx->ctrl_buf),
        .length = ctx->ctrl_buf_size,
...
// Return true on success.
bool NPRDMAAdapter::post_data_write(struct write_lat_mem *rem_mem)
{
    struct ibv_sge list;
...
bool NPRDMAAdapter::post_data_write_with_imm(struct write_lat_mem *rem_mem,
...
bool NPRDMAAdapter::connect_qp(struct write_lat_dest *my_dest,
...
bool NPRDMAAdapter::parse_recv_wc(struct ibv_wc *wc)
{
...
bool NPRDMAAdapter::parse_recv_with_imm_wc(struct ibv_wc *wc)
{
    if (wc->status != IBV_WC_SUCCESS)
    {
        fprintf(stderr, "Work request status is %s\n", ibv_wc_status_str(wc->status));
...
bool NPRDMAAdapter::parse_send_wc(struct ibv_wc *wc)
{
    if (wc->status != IBV_WC_SUCCESS)
    {
        fprintf(stderr, "Work request status is %s\n", ibv_wc_status_str(wc->status));
...
bool NPRDMAAdapter::parse_write_wc(struct ibv_wc *wc)
{
    if (wc->status != IBV_WC_SUCCESS)
    {
        fprintf(stderr, "Work request status is %s\n", ibv_wc_status_str(wc->status));
...
// Return true on success.
bool NPRDMAAdapter::wait_for_wc(struct ibv_wc *wc)
{
    while (true)
    {
