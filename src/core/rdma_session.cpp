...
RDMAEndpoint *RDMASession::pre_connect(PreConnector *pre)
{
    ...
    pre->pre_connect(); // using the socket for pre connecting by default
    std::this_thread::sleep_for(std::chrono::milliseconds(1));
    LOG(INFO) << "Waiting for pre connector for RDMA RDMA channel";
    this->pre_ = pre;
...
{
    RDMAEndpoint *new_endpoint = new RDMAEndpoint(pre);
